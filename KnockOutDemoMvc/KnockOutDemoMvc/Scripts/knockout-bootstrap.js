...
ko.bindingHandlers.typeahead = {
    init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
        var $element = $(element);
        var typeaheadArr = valueAccessor();
        var allBindings = allBindingsAccessor();
        function _initialize(typeaheadArr) {
            var options = {
                'local': (ko.isObservable(typeaheadArr.local) ? typeaheadArr.local() : typeaheadArr.local),
                'limit': typeaheadArr.limit || 10,
                'minLength': typeaheadArr.minLength,
                'valueKey': typeaheadArr.valueKey,
                'name': typeaheadArr.name,
            };
            if (typeaheadArr.template) {
                options.template = function (data) {
                    return typeaheadArr.template.replace(/{{([a-zA-Z_]+)}}/g, function (match, propName) {
                        if (propName in data) return data[propName];
                        return '';
                    });
                };
            }
        ...
        if (ko.isObservable(typeaheadArr.local)) {
            typeaheadArr.local.subscribe(function () {
                $element.typeahead('destroy');
                _initialize(typeaheadArr);
        ...
        $element.on('typeahead:selected', function () {
            ...
            $element.trigger('focus.tt');
        }).on('focus.tt', function () {
            $element.attr('placeholder', '');
            if ($element.val() == '') {
                setTimeout($element.typeahead('setQuery', ''), 0);
            }
        }).on('blur', function () {
            if ($element.attr('placeholder') == '') $element.attr('placeholder', $element.data('placeholder'));
        });
