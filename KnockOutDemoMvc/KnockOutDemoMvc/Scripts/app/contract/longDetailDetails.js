...
GMK.Contract.Details.ViewModel = function (commonModels, route, options) {
    ...
    self.initialize = function (callback) {
        base._get(options.getUrl, { id: self.id }, function (result) {
            ...
            var longDetail = self.item.WFLongContractDetail, instruments = longDetail.WFLongContractDetailInstruments(), len = instruments.length;
            longDetail.instrumentInfo = (len == 1) ? self.commonModels.findInstrument(instruments[0].WFInstrumentId()) :
                (len === 2 ? '合约A：{0}，分隔时间：{1}，合约B：{2}'.format(self.commonModels.findInstrument(instruments[0].WFInstrumentId()),
                moment(instruments[1].StartDate()).add(-1, 'd').format('YYYY-MM-DD'),
                self.commonModels.findInstrument(instruments[1].WFInstrumentId())) : '');
